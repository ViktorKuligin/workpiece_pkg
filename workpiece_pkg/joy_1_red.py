...
class JoyRed(Node):
    def __init__(self):
        super().__init__('joy_red')
        self.get_logger().warn("joy red start")
    ...
    def joy_cb(self, msg: Joy):
        ...
        for i in range(len(val_buttons)):
            if val_buttons[i] == 1:
                self.get_logger().info(f'buttons: {self.joy_red_button.get(i)}')
        ...
        for i in range(len(val_axes)):
            if val_axes[i] != 0.0:
                ...
                if val_axes[i] < 0:
                    key = '-'
                self.get_logger().info(f'axes: {self.joy_red_axes.get(key + str(i))}, val: {round(val_axes[i], 8)}')
